...
class DanhMucDoGiaViPage extends StatelessWidget {

  final SuaRepository suaRepository;

  const DanhMucDoGiaViPage({super.key, required this.suaRepository});
  ...
  Widget build(BuildContext context) {
    return BlocProvider(
      ...
      child: const DanhMucDoGiaViView(),
    );
  }
}


class DanhMucDoGiaViView extends StatefulWidget {
  const DanhMucDoGiaViView({super.key});

  @override
  State<DanhMucDoGiaViView> createState() => _DanhMucDoGiaViViewState();
}

class _DanhMucDoGiaViViewState extends State<DanhMucDoGiaViView> {
        ...
        children: [
          Padding(
            ...
            child: TextField(
              ...
              onChanged: (value) => context.read<SuaBloc>().add(SearchProductDanhMuc(value.trim(),7,userId)), // lưu thay đổi vào state, để tìm kiếm sản phẩm
              decoration: InputDecoration(
                ...
                hintText: 'Tìm kiếm tên theo đồ gia vị...',
                hintStyle: TextStyle(
                  fontSize: 13.sp,
                ),
              ...
              builder: (context, state) {
                ...
                if (isSearching) {
                  ...
                  if (state.statusSearch == StatusSearch.loading) {
                    ...
                    return const Center(child: Text('Không tìm thấy đồ gia vị.'));
                  }
                } else {
                  // LOAD BAN ĐẦU
                  if (state.statusLoadSua == StatusLoadSua.loading) {
                    ...
                    return const Center(child: Text('Không có đồ gia vị.'));
                  }
                }
