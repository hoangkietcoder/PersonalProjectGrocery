...
class DanhMucDoKhoPage extends StatelessWidget {

  final SuaRepository suaRepository;

  const DanhMucDoKhoPage({super.key, required this.suaRepository});
  ...
  Widget build(BuildContext context) {
    return BlocProvider(
      ...
      child: const DanhMucDoKhoView(),
    );
  }
}


class DanhMucDoKhoView extends StatefulWidget {
  const DanhMucDoKhoView({super.key});

  @override
  State<DanhMucDoKhoView> createState() => _DanhMucDoKhoViewState();
}

class _DanhMucDoKhoViewState extends State<DanhMucDoKhoView> {
        ...
        children: [
          Padding(
            ...
            child: TextField(
              ...
              onChanged: (value) => context.read<SuaBloc>().add(SearchProductDanhMuc(value.trim(),9,userId)), // lưu thay đổi vào state, để tìm kiếm sản phẩm
              decoration: InputDecoration(
                ...
                hintText: 'Tìm kiếm theo tên đồ khô...',
                hintStyle: TextStyle(
                  fontSize: 13.sp,
                ),
              ...
              builder: (context, state) {
                ...
                if (isSearching) {
                  ...
                  if (state.statusSearch == StatusSearch.loading) {
                    ...
                    return const Center(child: Text('Không tìm thấy đồ khô nào.'));
                  }
                } else {
                  // LOAD BAN ĐẦU
                  if (state.statusLoadSua == StatusLoadSua.loading) {
                    ...
                    return const Center(child: Text('Không có đồ khô .'));
                  }
                }
