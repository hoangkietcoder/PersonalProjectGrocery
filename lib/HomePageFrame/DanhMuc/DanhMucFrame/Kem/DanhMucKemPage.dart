...
class DanhMucKemPage extends StatelessWidget {

  final SuaRepository suaRepository;

  const DanhMucKemPage({super.key, required this.suaRepository});
  ...
  Widget build(BuildContext context) {
    return BlocProvider(
      ...
      child: const DanhMucKemView(),
    );
  }
}


class DanhMucKemView extends StatefulWidget {
  const DanhMucKemView({super.key});

  @override
  State<DanhMucKemView> createState() => _DanhMucKemViewState();
}

class _DanhMucKemViewState extends State<DanhMucKemView> {
        ...
        children: [
          Padding(
            ...
            child: TextField(
              ...
              onChanged: (value) => context.read<SuaBloc>().add(SearchProductDanhMuc(value.trim(),5,userId)), // lưu thay đổi vào state, để tìm kiếm sản phẩm
              decoration: InputDecoration(
                ...
                hintText: 'Tìm kiếm theo tên sản phẩm kem...',
                hintStyle: TextStyle(
                  fontSize: 13.sp,
                ),
              ...
              builder: (context, state) {
                ...
                if (isSearching) {
                  ...
                  if (state.statusSearch == StatusSearch.loading) {
                    ...
                    return const Center(child: Text('Không tìm thấy sản phẩm kem.'));
                  }
                } else {
                  // LOAD BAN ĐẦU
                  if (state.statusLoadSua == StatusLoadSua.loading) {
                    ...
                    return const Center(child: Text('Không có sản phẩm kem.'));
                  }
                }
